...
export function sanitizeError(msg: string): string {
  return msg.replace(/[A-Z_]{2,}=[^\s]+/g, "[REDACTED]").slice(0, 500)
}
...
function discoverPlugins(): string[] {
  ...
  return fs.readdirSync(PLUGINS_DIR).filter((f) => {
      ...
      ([db, leads]): PluginService => ({
        ...
        run: (name) =>
          Effect.gen(function* () {
            ...
            // Record the run
            const runResult = yield* db.run("INSERT INTO plugin_runs (plugin_name) VALUES (?)", name)
        ...
        listRuns: () => db.all<PluginRun>("SELECT * FROM plugin_runs ORDER BY started_at DESC"),
      }),
    ),
  ),
)
